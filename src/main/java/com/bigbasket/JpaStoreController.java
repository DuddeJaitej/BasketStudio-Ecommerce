...
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = "${app.frontend-url:http://localhost:8087}")
public class JpaStoreController {
    ...
    public JpaStoreController(UserRepository users, ProductRepository products, OrderRepository orders) {
        this.users = users;
    ...
    public List<ProductResponse> products(@RequestParam(required = false) String category) {
        List<ProductEntity> result = category == null || category.isBlank() ? products.findAll()
                : products.findByCategoryIgnoreCase(category);
        return result.stream().map(JpaStoreController::productResponse).toList();
    }

    @GetMapping("/products/{id}")
    public ProductResponse product(@PathVariable String id) {
        return productResponse(productOr404(id));
    }
    ...
    public ProductResponse createProduct(@Valid @RequestBody ProductRequest request) {
        ...
        return productResponse(products.save(new ProductEntity(id, request.name(), request.category(), request.price(),
                request.image(), request.tagline())));
    }
    ...
    public void deleteProduct(@PathVariable String id) {
        products.delete(productOr404(id));
    }
    ...
    public OrderResponse createOrder(@RequestHeader(value = "Authorization", required = false) String authorization,
        ...
        UserEntity user = userFor(authorization);
        OrderEntity order = new OrderEntity(
                "BB-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT), user,
                request.paymentMethod(),
                new OrderEntity.AddressData(request.address().name(), request.address().phone(),
                        request.address().line(), request.address().city(), request.address().state(),
                        request.address().pincode()));
        request.items().forEach(item -> order.addItem(productOr404(item.productId()), item.quantity()));
        return orderResponse(orders.save(order));
    }

    @GetMapping("/orders")
    @Transactional(readOnly = true)
    public List<OrderResponse> orderHistory(
            @RequestHeader(value = "Authorization", required = false) String authorization) {
        return orders.findByUserEmailOrderByCreatedAtDesc(userFor(authorization).getEmail()).stream()
                .map(this::orderResponse).toList();
    }
    ...
    public OrderResponse order(@PathVariable String id,
            @RequestHeader(value = "Authorization", required = false) String authorization) {
        return orderResponse(ownedOrder(id, userFor(authorization)));
    }
    ...
    public void deleteOrder(@PathVariable String id,
            @RequestHeader(value = "Authorization", required = false) String authorization) {
        orders.delete(ownedOrder(id, userFor(authorization)));
    }

    @GetMapping("/profile")
    public UserResponse profile(@RequestHeader(value = "Authorization", required = false) String authorization) {
        return userResponse(userFor(authorization), tokenFor(authorization));
    }

    private UserResponse loginUser(UserEntity user, String password) {
        if (!user.getPasswordHash().equals(hash(password)))
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Email or password is incorrect");
        String token = UUID.randomUUID().toString();
        sessions.put(token, user.getEmail());
        return userResponse(user, token);
    }

    private UserEntity userFor(String authorization) {
        String email = tokenEmail(authorization);
        return users.findByEmailIgnoreCase(email)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED, "User account not found"));
    }

    private String tokenEmail(String authorization) {
        if (authorization == null || !authorization.startsWith("Bearer "))
    ...
    private String tokenFor(String authorization) {
        return authorization.substring(7);
    }

    private OrderEntity ownedOrder(String id, UserEntity user) {
        return orders.findByIdAndUserEmail(id, user.getEmail())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Order not found"));
    }

    private ProductEntity productOr404(String id) {
        return products.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Product not found"));
    }

    private static ProductResponse productResponse(ProductEntity p) {
        return new ProductResponse(p.getId(), p.getName(), p.getCategory(), p.getPrice(), p.getImage(), p.getTagline());
    }

    private static UserResponse userResponse(UserEntity u, String token) {
        return new UserResponse(u.getName(), u.getEmail(), u.getPhone(), u.getDateOfBirth().toString(), token);
    }

    private OrderResponse orderResponse(OrderEntity order) {
        List<OrderItemResponse> items = order.getItems().stream()
                .map(item -> new OrderItemResponse(item.getProduct().getId(), item.getProduct().getName(),
                        item.getProduct().getImage(), item.getQuantity(), item.getUnitPrice()))
                .toList();
        return new OrderResponse(order.getId(), order.getStatus(), order.getPaymentMethod(),
                order.getCreatedAt().toString(), items,
                items.stream().mapToInt(item -> item.quantity() * item.unitPrice()).sum());
    }
    ...
    record OrderResponse(String id, String status, String paymentMethod, String createdAt,
            List<OrderItemResponse> items, int total) {
    }
}
